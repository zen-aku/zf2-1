...
declareVars() - создать(задекларировать) свойство-переменную вида.
flashMessenger() - вывести сообщение, сгенерированное плагином контроллеров flashMessenger()
        Мегаполезный хелпер. Хелпер номер 1!!!
...
identity() - проверка идертификации пользователя.
json() - кодирование и вывод json-данных
layout() - вывод другого шаблона

partial() - подключает шаблон в вид и передаёт этому шаблону массив параметров во втором аргументе хелпера.
partialLoop() - многократно подключает шаблон в вид и передаёт этому шаблону итерируемые параметры во втором аргументе
        Очень полезные хелперы. Позволяют разгрузить шаблоны повторяющимися кодами со вставками php-параметров.
