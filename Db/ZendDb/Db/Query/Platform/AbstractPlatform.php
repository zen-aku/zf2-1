...
namespace Zend\Db\Query\Platform;
...
class AbstractPlatform implements PlatformDecoratorInterface, PreparableQueryInterface, QueryInterface
{
    /**
     * @var object
     */
    ...
    {
        if (!$this->subject instanceof PreparableQueryInterface) {
            throw new Exception\RuntimeException('The subject does not appear to implement Zend\Db\Query\PreparableQueryInterface, thus calling prepareStatement() has no effect');
        }

        $decoratorForType = false;
        foreach ($this->decorators as $type => $decorator) {
            if ($this->subject instanceof $type && $decorator instanceof PreparableQueryInterface) {
                /** @var $decoratorForType PreparableQueryInterface|PlatformDecoratorInterface */
    ...
    {
        if (!$this->subject instanceof QueryInterface) {
            throw new Exception\RuntimeException('The subject does not appear to implement Zend\Db\Query\PreparableQueryInterface, thus calling prepareStatement() has no effect');
        }

        $decoratorForType = false;
        foreach ($this->decorators as $type => $decorator) {
            if ($this->subject instanceof $type && $decorator instanceof QueryInterface) {
                /** @var $decoratorForType QueryInterface|PlatformDecoratorInterface */
                $decoratorForType = $decorator;
                break;
            }
        }
