    ...
    public function prepareStatement(AdapterInterface $adapter, StatementContainerInterface $statementContainer)
    {
        ...
        foreach ($this->set as $column => $value) {
            if ($value instanceof Expression) {
                ...
                $setSql[] = $driver->quoteIdentifier($column) . ' = ' . $exprData->getSql();
                $parameterContainer->merge($exprData->getParameterContainer());
            } else {
                $setSql[] = $driver->quoteIdentifier($column) . ' = ' . $driver->formatParameterName($column);
    ...
    public function getSqlString(DriverInterface $driver)
    {
        ...
        foreach ($this->set as $column => $value) {
            if ($value instanceof ExpressionInterface) {
                ...
                $setSql[] = $driver->quoteIdentifier($column) . ' = NULL';
            } else {
                $setSql[] = $driver->quoteIdentifier($column) . ' = ' . $driver->quoteValue($value);
            }
        }
        $set = implode(', ', $setSql);
