...
abstract class AbstractSql
{
    ...
    protected function processExpression( ExpressionInterface $expression, DriverInterface $driver = null, $namedParameterPrefix = null ) {
        ...
        foreach ($parts as $part) {
            ...
            foreach ($values as $vIndex => $value) {
                if (isset($types[$vIndex]) && $types[$vIndex] == ExpressionInterface::TYPE_IDENTIFIER) {
                    $values[$vIndex] = $driver->quoteIdentifierInFragment($value);
                } elseif (isset($types[$vIndex]) && $types[$vIndex] == ExpressionInterface::TYPE_VALUE && $value instanceof Select) {
                    // process sub-select              
                    $values[$vIndex] = '(' . $this->processSubSelect($value, $driver, $parameterContainer) . ')';                   
                    ...
                    $innerStatementContainer = $this->processExpression($value, $driver, $namedParameterPrefix . $vIndex . 'subpart');
                    ...
                    $values[$vIndex] = $driver->quoteValue($value);
	...
	/**
	 * @param \Zend\Db\Sql\Select $subselect
	 * @param DriverInterface $driver
	 * @param ParameterContainer $parameterContainer
	 * @return type
	 */
    protected function processSubSelect(Select $subselect, DriverInterface $driver = null, ParameterContainer $parameterContainer = null) {
        if ($driver) {
			...
			$subselect->prepareStatement(new \Zend\Db\Adapter\Adapter($driver), $stmtContainer);
            ...
            $sql = $subselect->getSqlString($driver);         
        }
        return $sql;
    }
}
