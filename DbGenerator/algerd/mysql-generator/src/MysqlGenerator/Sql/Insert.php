...
use MysqlGenerator\Adapter\DriverInterface;
    ...
    public function prepareStatement(AdapterInterface $adapter, StatementContainerInterface $statementContainer){
        ...
        if (is_array($this->values)) {
            foreach ($this->columns as $cIndex => $column) {
                $columns[$cIndex] = $driver->quoteIdentifier($column);
                if (isset($this->values[$cIndex]) && $this->values[$cIndex] instanceof Expression) {
                    $exprData = $this->processExpression($this->values[$cIndex], $driver);
            ...
            $columns = array_map(array($driver, 'quoteIdentifier'), $this->columns);
        ...
        if (is_array($this->values)) {
            ...
            foreach ($this->values as $value) {
                if ($value instanceof Expression) {
                    $exprData = $this->processExpression($value, $driver);
                    $values[] = $exprData->getSql();
                } elseif ($value === null) {
                    $values[] = 'NULL';
                } else {
                    $values[] = $driver->quoteValue($value);
                }
            }
            return sprintf(
                $this->specifications[static::SPECIFICATION_INSERT],
            ...
            $selectString = $this->values->getSqlString($driver);
            if ($columns) {
                $columns = "($columns)";
            }
            return sprintf(
