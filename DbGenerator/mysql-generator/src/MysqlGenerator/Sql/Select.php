...
class Select extends AbstractSql implements SqlInterface, PreparableSqlInterface {
    ...
    public function prepareStatement(AdapterInterface $adapter, StatementContainerInterface $statementContainer){
        ...
        foreach ($this->specifications as $name => $specification) {
            $parameters[$name] = $this->{'process' . $name}($adapter, $parameterContainer, $sqls, $parameters);
    ...
    protected function processSelect(AdapterInterface $adapter = null, ParameterContainer $parameterContainer = null){
        ...
        if ($this->quantifier) {
            if ($this->quantifier instanceof ExpressionInterface) {
                $quantifierParts = $this->processExpression($this->quantifier, $adapter, 'quantifier');
                if ($parameterContainer) {
     ...
     * @return
     */
    protected function processJoins(AdapterInterface $adapter = null, ParameterContainer $parameterContainer = null){
        if (!$this->joins) {
            return null;
        }
        ...
        foreach ($this->joins as $j => $join) {
            ...
            $joinSpecArgArray[$j][] = ($join['on'] instanceof ExpressionInterface)
                ? $this->processExpression($join['on'], $adapter, $this->processInfo['paramPrefix'] . 'join' . ($j+1) . 'part')
     ...
     * @return
     */
    protected function processWhere(AdapterInterface $adapter = null, ParameterContainer $parameterContainer = null){
        if ($this->where->count() == 0) {
            return null;
        }
        $whereParts = $this->processExpression($this->where, $adapter, $this->processInfo['paramPrefix'] . 'where');
        if ($parameterContainer) {
     ...
     * @return
     */
    protected function processGroup(AdapterInterface $adapter = null, ParameterContainer $parameterContainer = null){
        if ($this->group === null) {
            return null;
        }
        // process table columns
        ...
        foreach ($this->group as $column) {
            ...
            if ($column instanceof Expression) {
                $columnParts = $this->processExpression($column, $adapter, $this->processInfo['paramPrefix'] . 'group');
                if ($parameterContainer) {
                    $parameterContainer->merge($columnParts->getParameterContainer());
                }
     ...
     * @return
     */
    protected function processHaving(AdapterInterface $adapter = null, ParameterContainer $parameterContainer = null){
        if ($this->having->count() == 0) {
            return null;
        }
        $whereParts = $this->processExpression($this->having, $adapter, $this->processInfo['paramPrefix'] . 'having');
        if ($parameterContainer) {
            $parameterContainer->merge($whereParts->getParameterContainer());
        }
        return array($whereParts->getSql());
     ...
     * @return
     */
    protected function processOrder(AdapterInterface $adapter = null, ParameterContainer $parameterContainer = null){
        if (empty($this->order)) {
            return null;
        }
        $orders = array();
        foreach ($this->order as $k => $v) {
            if ($v instanceof Expression) {
                ...
                $orderParts = $this->processExpression($v, $adapter);
                if ($parameterContainer) {
     ...
     * @return string
     */
    protected function processLimit(AdapterInterface $adapter = null, ParameterContainer $parameterContainer = null){
        if ($this->limit === null && $this->offset !== null) {
            return array('');
        }
        if ($this->limit === null) {
     ...
     * @return string
     */
    protected function processOffset(AdapterInterface $adapter = null, ParameterContainer $parameterContainer = null){
        if ($this->offset === null) {
            return null;
        }
        if ($adapter) {
     ...
     * @return
     */
    protected function processCombine(AdapterInterface $adapter = null, ParameterContainer $parameterContainer = null){
        if ($this->combine == array()) {
            return null;
        }
        ...
        return array(
            $type,
            $this->processSubSelect($this->combine['select'], $adapter)
        );
    }
