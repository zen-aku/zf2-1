...
class Select extends AbstractSql implements SqlInterface, PreparableSqlInterface {
    ...
    protected function processWhere(AdapterInterface $adapter = null, ParameterContainer $parameterContainer = null){
        ...
        $whereParts = $this->processExpression($this->where, $adapter, true, $this->processInfo['paramPrefix'] . 'where');
        if ($parameterContainer) {
    ...
    protected function processGroup(AdapterInterface $adapter = null, ParameterContainer $parameterContainer = null){
        ...
        foreach ($this->group as $column) {
            ...
            if ($column instanceof Expression) {
                $columnParts = $this->processExpression($column, $adapter, true, $this->processInfo['paramPrefix'] . 'group');
                if ($parameterContainer) {
                    $parameterContainer->merge($columnParts->getParameterContainer());
                }
    ...
    protected function processHaving(AdapterInterface $adapter = null, ParameterContainer $parameterContainer = null){
        ...
        $whereParts = $this->processExpression($this->having, $adapter, true, $this->processInfo['paramPrefix'] . 'having');
        if ($parameterContainer) {
            $parameterContainer->merge($whereParts->getParameterContainer());
        }
        return array($whereParts->getSql());
    ...
    protected function processOrder(AdapterInterface $adapter = null, ParameterContainer $parameterContainer = null){
        ...
        foreach ($this->order as $k => $v) {
            if ($v instanceof Expression) {
                ...
                $orderParts = $this->processExpression($v, $adapter, true);
                if ($parameterContainer) {
     ...
     * @return
     */
    protected function processCombine(AdapterInterface $adapter, ParameterContainer $parameterContainer = null){
        if ($this->combine == array()) {
            return null;
        }
